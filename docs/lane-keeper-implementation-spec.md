...
`readiness check`, `readiness await`, `branch name`, and `mr render`.
...
- validate every configured check predicate's Starlark syntax structurally
  from the parsed model, independent of TOML string quoting style (basic,
  literal, or multi-line);
- locate ordinary triple-quoted predicates beneath
  `[_.lane-keeper.checks.<name>]` by text position for `--fmt` in-place
  splicing;
...
`--fmt` in-place formatting remains limited to the documented ordinary
triple-quoted representation, since Buildifier splicing requires literal text
positions. `--lint` predicate syntax validation has no such limitation: it
validates directly from the parsed configuration model.
...
For a globally installed local binary, a different tool version produces an
advisory warning to stderr if the repository pins another version in
`[tools].lane-keeper`, for example:

```text
lane-keeper: warning: running version 1.0.0, but repository pins 9.9.9
```

The warning is non-fatal: the command continues and its exit code is
unaffected. A `dev` build (the default when `version` is not set via linker
flags) is treated as a local build, not a mismatched release, and is never
warned about.
...
Golden output is used for `readiness check`, `branch name`, and `mr render`
human-readable and JSON output, comparing against fixed files in
`cmd/lane-keeper/testdata/*.golden`. Non-deterministic output (commit SHAs,
timestamps) is normalized to stable placeholders (`SHA`, `DATE`) before
comparison. Golden files are regenerated by running the golden tests with
`UPDATE_GOLDEN=1`.
